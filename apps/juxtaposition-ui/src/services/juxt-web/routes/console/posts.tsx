...
postsRouter.get('/:post_id/oembed.json', async function (req, res) {
	...
	const { data: post } = await req.api.getPostsPostId({ post_id: params.post_id });
	if (!post) {
...
postsRouter.post('/empathy', yeahLimit, async function (req, res) {
	...
	const { data: post } = await req.api.getPostsPostId({ post_id: body.postID });
	if (!post) {
		return res.sendStatus(404);
	}
	...
	const { data: result } = await req.api.postPostsPostIdEmpathies({ post_id: post.id, action });
	if (result === null) {
...
postsRouter.get('/:post_id', async function (req, res) {
	...
	if (post.parent) {
		const { data: parent } = await req.api.getPostsPostId({ post_id: post.parent });
		if (!parent) {
			return res.redirect('/404');
		}
	...
	const replies = (await req.api.getPosts({ parent_id: post.id, include_replies: 'true' }))?.data.items ?? [];
	const postPNID = await getUserAccountData(post.pid);
...
postsRouter.delete('/:post_id', async function (req, res) {
	...
	const { data: result } = await req.api.deletePostsPostId({ post_id: post.id, reason: query.reason });
	if (result === null) {
		return res.sendStatus(404);
	}
...
postsRouter.get('/:post_id/create', async function (req, res) {
	...
	const { data: parent } = await req.api.getPostsPostId({ post_id: params.post_id });
	if (!parent) {
		return res.sendStatus(404);
	}
...
postsRouter.post('/:post_id/report', upload.none(), async function (req, res) {
	...
	const { data: post } = await req.api.getPostsPostId({ post_id });
