...
communitiesRouter.post('/follow', upload.none(), async function (req, res) {
	...
	const self = auth().self;
	const isFollowing = self.content.followed_communities.includes(community.olive_community_id);

	let result: FollowAction;
	if (isFollowing) {
		const { data: followResult } = await req.api.communities.unfollow({ id: body.id });
		result = followResult;
	} else {
		const { data: followResult } = await req.api.communities.follow({ id: body.id });
		result = followResult;
	}
	...
	res.send({
		status: 200,
		id: result.id,
		action: result.action === 'follow' ? 'follow' : 'unfollow',
